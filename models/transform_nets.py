...
def feature_transform_net(inputs, is_training, bn_decay=None, K=64, reuse = False):
    ...
    net = tf_util.conv2d(inputs, 64, [1,1],
                         ...
                         bn=True, is_training=is_training,
                         scope='tconv1', bn_decay=bn_decay)
    print "Feature Transform.shape: ",net.shape
...
def quick_feature_transform_net(inputs, is_training, bn_decay=None, K=64):
    """ Feature Transform Net, input is BxNx1xK
        Return:
            Transformation matrix of size KxK """
    batch_size = inputs.get_shape()[0].value
    ...
    net = tf_util.conv2d(inputs, 8, [1,1],
                         padding='VALID', stride=[1,1],
                         bn=True, is_training=is_training,
                         scope='tconv1', bn_decay=bn_decay)
    print "Feature Transform.shape: ",net.shape
...
def dgcnn_input_transform_net(edge_feature, is_training, bn_decay=None, K=3, is_dist=False):
    ...
    print "Input Transform First layer: ",net.shape

    net = tf_util.conv2d(net, 128, [1,1],
                padding='VALID', stride=[1,1],
                bn=True, is_training=is_training,
                scope='tconv2', bn_decay=bn_decay, is_dist=is_dist)
    print "Input Transform Second layer: ",net.shape

    net = tf.reduce_max(net, axis=-2, keep_dims=True)
    print "Input Transform Third layer: ",net.shape
    ...
    net = tf_util.max_pool2d(net, [num_point,1],
                padding='VALID', scope='tmaxpool')
    print "Input Transform Fourth layer: ",net.shape
    ...
    print "Input Transform FC layer: ",net.shape
    with tf.variable_scope('transform_XYZ') as sc:
    # assert(K==3)
    ...
    transform = tf.reshape(transform, [batch_size, K, K])
    print "Input Transform last layer: ",transform.shape
    return transform


def quick_dgcnn_input_transform_net(edge_feature, is_training, bn_decay=None, K=3, is_dist=False):
    batch_size = edge_feature.get_shape()[0].value
    num_point = edge_feature.get_shape()[1].value

    # input_image = tf.expand_dims(point_cloud, -1)
    net = tf_util.conv2d(edge_feature, 8, [1,1],
                padding='VALID', stride=[1,1],
                bn=True, is_training=is_training,
                scope='tconv1', bn_decay=bn_decay, is_dist=is_dist)
    print "Input Transform First layer: ",net.shape
